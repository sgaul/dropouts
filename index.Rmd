...
dropoutsdistrict <- ddply(dropouts,.(School.Year, District.Name),summarise,
                        wm=weighted.mean(Annual.Dropout.Rate,Enrollment,na.rm=TRUE))
dropoutsdistrict <- cast(dropoutsdistrict, District.Name ~ School.Year)
names(dropoutsdistrict)[2:8] <- c("X04","X05","X06","X07","X08","X09","X10")
dropoutsdistrict$District.Name <- reorder(dropoutsdistrict$District.Name, dropoutsdistrict$X10)
library(grid)
ggplot(data = dropoutsdistrict, aes(y = District.Name, yend = District.Name, x = X04, xend = X10)) + 
  geom_segment(arrow = arrow(type = "open"), aes(group = District.Name)) + 
  labs(y = NULL, x = "Dropout rate") + 
...
dropoutsschool <- cast(dropouts, School.Name ~ School.Year, value = "Annual.Dropout.Rate")
names(dropoutsschool)[2:8] <- c("X04","X05","X06","X07","X08","X09","X10")
dropoutsschool$School.Name <- reorder(dropoutsschool$School.Name, dropoutsschool$X10)
library(grid)
ggplot(data = subset(dropoutsschool, X10 >= 0), aes(y = School.Name, yend = School.Name, x = X04, xend = X10)) + 
  geom_segment(arrow = arrow(type = "open"), aes(group = School.Name)) + 
  geom_point(aes(x = X10)) +
  labs(y = NULL, x = "Dropout rate") + 
  geom_vline(xintercept = 1.97, alpha = 0.2) + 
  theme_minimal()
```
